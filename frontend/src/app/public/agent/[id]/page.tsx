...
function ChatContent() {
  ...
  const send = async () => {
    ...
    try {
      ...
      const res = await fetch(`${backend}/api/chat/public`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ agentId, message: t, sessionId }),
      });
      const data = await res.json();
      if (res.ok)
        setMessages((prev) => [...prev, { type: "agent", content: data.message || "" }]);
      else
        setMessages((prev) => [
          ...prev,
          { type: "agent", content: data.error || "Something went wrong." },
        ]);
    } catch {
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, overflow: "auto", padding: 16 }}>
        {messages.length === 0 && (
          <div style={{ textAlign: "center", color: sub, marginTop: 40 }}>
      ...
      >
        <Input
          ...
          className="flex-1 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-gray-500"
        />
        <Button
          onClick={send}
          disabled={loading}
