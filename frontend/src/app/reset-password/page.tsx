...
export default function ResetPasswordPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (res.success) { setMessage('Password reset. Redirecting to login...'); setTimeout(()=>router.push('/login'),1500); }
        ...
        <CardContent>
          <form onSubmit={submit} className="space-y-4">
            <div className="relative">
              ...
              <Input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="New password" className="pl-10 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-gray-500" />
            </div>
